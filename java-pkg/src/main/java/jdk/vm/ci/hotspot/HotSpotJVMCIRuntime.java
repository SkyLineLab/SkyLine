...
public final class HotSpotJVMCIRuntime implements JVMCIRuntime {
    ...
    @NativeImageReinitialize private Object[] excludeFromJVMCICompilation;
    ...
    private HotSpotResolvedJavaType fromClass0(Class<?> javaClass) {
        WeakReferenceHolder<HotSpotResolvedJavaType> ref = new WeakReferenceHolder<>(createClass(javaClass));
    ...
    static int writeDebugOutput0(CompilerToVM vm, byte[] bytes, int offset, int length, boolean flush, boolean canThrow) {
        ...
        if (length <= 8) {
            ...
            UnsafeUtil unsafe = UnsafeAccess.UNSAFE;
            long buffer = unsafe.allocateMemory(length);
            try {
                unsafe.copyMemory(bytes, UnsafeAccess.UNSAFE.arrayBaseOffset(byte[].class), null, buffer, length);
     ...
     */
    public void excludeFromJVMCICompilation(Object... modules) {
