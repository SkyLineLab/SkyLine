...
final class HotSpotCompiledCodeStream implements AutoCloseable {
    ...
    private static final int HEADER = UnsafeAccess.UNSAFE.addressSize() + 4;
    ...
    private final UnsafeUtil unsafe = UnsafeAccess.UNSAFE;
     ...
     * IdentityHashMap#NULL_KEY constant.
     */
    ...
    private int getDataSize(long chunk) {
        int sizeOffset = UnsafeAccess.UNSAFE.addressSize();
    ...
    private void dump(String dumpName) {
        ...
        for (long c = headChunk; c != 0; c = getChunkNext(c)) {
            ...
            unsafe.copyMemory(null, data0, data, UnsafeUtil.UNSAFE_UTIL.arrayBaseOffset(byte[].class), dataSize);
